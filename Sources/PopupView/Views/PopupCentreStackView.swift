...
struct PopupCentreStackView: View {
    ...
    var body: some View {
        createPopup()
            ...
            .transition(.asymmetric(insertion: .opacity.animation(transitionAnimation), removal: .scale(scale: 0.9).combined(with: .opacity).animation(items.count == 1 ? transitionAnimation : nil)))
