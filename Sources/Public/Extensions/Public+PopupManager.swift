...
public extension PopupManager {
    ...
    static func showAndStack(_ popup: some Popup) { performOperation(.insertAndStack(.init(popup))) }

    /// Displays the popup. Closes previous one
    static func showAndReplace(_ popup: some Popup) { performOperation(.insertAndReplace(.init(popup))) }
}
public extension PopupManager {
    ...
    static func dismissPopup(id: String) { performOperation(.remove(.init(value: id))) }

    /// Dismisses all the popups of provided type on the stack
    static func dismissPopup<P: Popup>(_ popup: P.Type) { performOperation(.remove(ID(popup))) }
